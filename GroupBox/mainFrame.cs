...
namespace L25_GroupBox
{
    public partial class mainFrame : Form
    {
        ...
        private void btnSignUp_Click( object sender, EventArgs e )
        {
            ...
            foreach ( RadioButton item in gbPaymentMethods.Controls )
            {
                if ( item.Checked )
                {
                    paymentMethod = item.Text;
                }
            }

            if ( gender != "" && paymentMethod != "" )
                MessageBox.Show( $"You've selected gender: {gender} and payment method: {paymentMethod}" );
        }
    }
}
